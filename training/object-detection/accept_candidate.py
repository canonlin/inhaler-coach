...
def compare_reports(baseline: dict, candidate: dict, minimum_recall_gain: float) -> dict:
	"""Return the promotion decision and the evidence used to make it."""
	recall_gain = candidate["frame_recall"] - baseline["frame_recall"]
	false_fire_ok = candidate["false_fires"] <= baseline["false_fires"]
	accepted = recall_gain >= minimum_recall_gain and false_fire_ok
	return {
		"accepted": accepted,
...
def main() -> None:
	...
	result = compare_reports(baseline, candidate, args.minimum_recall_gain)
